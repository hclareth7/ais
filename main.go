package main

import (
	"context"
	"embed"
	"fmt"
	"os"
	"path/filepath"
	"runtime"
	"strings"

	"github.com/hclareth7/ais/internal/fileserver"
	"github.com/hclareth7/ais/internal/platform"
	"github.com/wailsapp/wails/v2"
	"github.com/wailsapp/wails/v2/pkg/options"
	"github.com/wailsapp/wails/v2/pkg/options/assetserver"
	"github.com/wailsapp/wails/v2/pkg/options/linux"
	"github.com/wailsapp/wails/v2/pkg/options/mac"
	"github.com/wailsapp/wails/v2/pkg/options/windows"
)

//go:embed all:frontend/dist
var assets embed.FS

//go:embed build/appicon.png
var appIcon []byte

var version = "dev"

func main() {
	args := os.Args[1:]
	rootPath := "."
	enablePipe := false
	pipePath := ""

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--version", "-v":
			fmt.Printf("ais %s\n", version)
			os.Exit(0)
		case "--mcp":
			fmt.Println("MCP mode not yet implemented")
			os.Exit(0)
		case "--pipe":
			enablePipe = true
			if i+1 < len(args) && len(args[i+1]) > 0 && args[i+1][0] != '-' {
				i++
				pipePath = args[i]
			}
		default:
			if len(args[i]) > 0 && args[i][0] != '-' {
				rootPath = args[i]
			} else {
				fmt.Fprintf(os.Stderr, "unknown flag: %s\n", args[i])
				os.Exit(1)
			}
		}
	}

	absPath, err := filepath.Abs(rootPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error resolving path: %v\n", err)
		os.Exit(1)
	}
	absPath, err = filepath.EvalSymlinks(absPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error resolving symlinks: %v\n", err)
		os.Exit(1)
	}

	initialFile := ""

	info, err := os.Stat(absPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Path not found: %s\n", absPath)
		os.Exit(1)
	}
	if !info.IsDir() {
		// If the path is a file, check if it's a markdown file
		ext := strings.ToLower(filepath.Ext(absPath))
		if ext != ".md" && ext != ".markdown" {
			fmt.Fprintf(os.Stderr, "Not a markdown file: %s\n", absPath)
			os.Exit(1)
		}
		initialFile = filepath.Base(absPath)
		absPath = filepath.Dir(absPath)

		// Re-stat the directory
		info, err = os.Stat(absPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Directory not found: %s\n", absPath)
			os.Exit(1)
		}
		if !info.IsDir() {
			fmt.Fprintf(os.Stderr, "Parent is not a directory: %s\n", absPath)
			os.Exit(1)
		}
		// Re-resolve symlinks for the directory
		absPath, err = filepath.EvalSymlinks(absPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error resolving symlinks: %v\n", err)
			os.Exit(1)
		}
	}

	app := NewApp(absPath, initialFile)
	fsHandler := fileserver.NewHandler(app.GetRootPath)

	err = wails.Run(&options.App{
		Title:            "ais",
		Width:            1200,
		Height:           800,
		MinWidth:         600,
		MinHeight:        400,
		WindowStartState: options.Maximised,
		Frameless:        runtime.GOOS != "darwin",
		AssetServer: &assetserver.Options{
			Assets:  assets,
			Handler: fsHandler,
		},
		BackgroundColour: &options.RGBA{R: 0, G: 0, B: 0, A: 0},
		Linux: &linux.Options{
			Icon:                appIcon,
			WindowIsTranslucent: true,
			ProgramName:         "ais",
		},
		Mac: &mac.Options{
			TitleBar: &mac.TitleBar{
				TitlebarAppearsTransparent: true,
				HideTitle:                  true,
				HideTitleBar:               false,
				FullSizeContent:            true,
			},
			WebviewIsTransparent: true,
			WindowIsTranslucent:  true,
			About: &mac.AboutInfo{
				Title:   "ais",
				Message: "Ambient Intuition — markdown reading surface",
				Icon:    appIcon,
			},
		},
		Windows: &windows.Options{
			WebviewIsTransparent: true,
			WindowIsTranslucent:  true,
		},
		OnStartup: app.startup,
		OnDomReady: func(_ context.Context) {
			platform.ConfigureWindow(20)
			if enablePipe {
				path, err := app.StartPipe(pipePath)
				if err != nil {
					fmt.Fprintf(os.Stderr, "warning: failed to start pipe: %v\n", err)
				} else {
					fmt.Fprintf(os.Stdout, "%s\n", path)
				}
			}
		},
		OnShutdown: app.shutdown,
		Bind: []any{
			app,
		},
	})

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		os.Exit(1)
	}
}
